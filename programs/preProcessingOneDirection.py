...
preProcessingDict="preProcessingDict"
computationDict="computationDict"
...
nbProcConstruct=int(computation_dictionary.get('"nb_proc_meshing"'))
nbProcRun=int(computation_dictionary.get('"nb_proc_computation"'))
solver=computation_dictionary.get('"solver"')
...
snappy_enable=computation_dictionary.get('"snappy_enable"')
...
dirEtude=preProcessing_dictionary.get('"path_study"')
...
for angle in angle_list:
    ...
    zmaxAndZmin=getBoundaries.getHeightBuildingAndZmaxAndZmin(directory,meshingMaxSize,nbLayer,refinementGround)
    ...
    for i in range(0,nb_inlet_polluant):
        ...
        lines_fvOptionsOutput.append("        {\n")
        lines_fvOptionsOutput.append("	  	      s	       ("+str(valeurs_polluant_list[i])+" 0);\n")
        lines_fvOptionsOutput.append("        }\n")
        lines_fvOptionsOutput.append("    }\n")
        lines_fvOptionsOutput.append("}\n\n")
    ...
    allrunParallelMeshingFile=open(directory+"/allrunParallelMeshing"+str(snappy_enable),"w")
    print(directory)
