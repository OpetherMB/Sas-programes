...
def get_concat_v_blank(im1,im2,im3,pollutant_name,scale,number_of_lines=5,color=(255,255,255),filter_value=0):
    ...
    draw.text(xy=(int(im2.width*(1+ratio_legende_logo_l))+int(im3.width/(number_of_lines-1)),im1.height+int(im1.height*separation_image)+int(im2.height*ratio_legende_logo_h)+int(im3.height*1.9)),text=str("Concentration en "+str(pollutant)+" (µg/m3)"),fill=(0,0,0),font=font_type_legend)
        ...
        for map_csv_format in map_csv_format_list :
            ...
            if(len(rgb_matrixes)!=1):
                ...
                for i,matrix in enumerate(rgb_matrixes):
                    ...
                    get_concat_v_blank(Image.fromarray(matrix).resize(size_image),image_logo,image_color_bar,pollutant,scale).save(path_map_out_final+"."+format_image)
                    
                    
            else:
                get_concat_v_blank(Image.fromarray(rgb_matrixes[0]).resize(size_image),image_logo,image_color_bar,pollutant,scale).save(path_map_out+"."+format_image)
