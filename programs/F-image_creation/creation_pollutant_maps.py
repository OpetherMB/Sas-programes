...
def get_concat_v_blank(im1,im2,im3,pollutant_name,height,scale,minimal_height=0,number_of_lines=5,color=(255,255,255),filter_value=0):
    """
    number of lines correspond au nombre de valeur dans la légende
    height la hauteur de la simu
    """
    
    #séparation entre l'image de pollution et la légende
    separation_image=0.005
    ...
    font_type_number = ImageFont.truetype('arial.ttf',int(round(20*im1.height/800)))
    font_type_legend = ImageFont.truetype('arial.ttf',int(round(18*im1.height/800)))
    if(filter_value>0):
        im1=im1.filter(ImageFilter.MedianFilter(size=filter_value))
    dst=Image.new('RGB',(max(im1.width,im2.width),im1.height + int(im1.height*separation_image) + im2.height),color)
    dst.paste(im1,(0,0))
    ...
    draw.line((0,im1.height)+im1.size,width=int(im1.height*separation_image),fill=(0,0,0))
    for i in range(0,number_of_lines):
        
        draw.text(xy=(int(im2.width*(1+ratio_legende_logo_l*0.3))+int(i*im3.width/(number_of_lines-1)),im1.height+int(im1.height*separation_image)+int(im2.height*ratio_legende_logo_h)+int(im3.height*1.05)),text=str(round(scale[0]+i*(scale[1]-scale[0])/(number_of_lines-1),1)),fill=(0,0,0),font=font_type_number)
    
    draw.text(xy=(int(im2.width*(1+ratio_legende_logo_l))+int(im3.width/(number_of_lines)),im1.height+int(im1.height*separation_image)+int(im2.height*ratio_legende_logo_h)+int(im3.height*1.9)),text=str("Concentration en "+str(pollutant)+" à "+str(float(height)-minimal_height)+"m (µg/m3)"),fill=(0,0,0),font=font_type_legend)
...
def ExpansionHistogram(x,minPol,maxPol,hsl_range_min,hsl_range_max):
    ...
    hsl_degree_value=(hsl_range_max-hsl_range_min)-(x-minPol)*(hsl_range_max-hsl_range_min)/(maxPol-minPol)+hsl_range_min
    
#    if(x<=minPol*1.01):
#        hsl_degree_value=240
    """ La fonction renvoie un chiffre situé entre 0 (rouge) et 240(bleu foncé) suivant le code hsl (hue saturation light)"""
    return hsl_degree_value
...
parser.add_argument('-min_h',type=float,default=0, help="change the referential of the height from simulation to adapt if the road is not at the same height at the ground of the area of interest")
...
minimal_height=float(args.min_h)
...
#paramètre pour l'emplacement du logo et de la légende
Image_tier_horizontal=int(round(1/3*size_image[1]))
Image_2tier_horizontal=int(round(2/3*size_image[1]))
Image_tier_vertical=int(round(Image_tier_horizontal*image_logo.height/image_logo.width))

#resize de l'image
image_logo=image_logo.resize((Image_tier_horizontal,Image_tier_vertical))
# Création de la barre de couleur pour le scale de l'image
...
for pollutant in pollutant_directories :
    ...
    # sélection du scale selon le polluant
    if("NO2" in pollutant):
        scale=scale_NO2
    ...
    print("minimal height :",minimal_height)
    print("")    
    ...
    for mean in mean_directories:
        ...
        for map_csv_format in map_csv_format_list :
            ...
            # print("lama",map_csv_format.split('_')[2].split(".")[0])
            height=map_csv_format.split('_')[-1].replace(".csv","").replace("m","")
            ...
            if(len(rgb_matrixes)!=1):
                ...
                for i,matrix in enumerate(rgb_matrixes):
                    ...
                    get_concat_v_blank(Image.fromarray(matrix).resize(size_image),image_logo,image_color_bar,pollutant,height,scale,minimal_height).save(path_map_out_final+"."+format_image)
                    
                    
            else:
                get_concat_v_blank(Image.fromarray(rgb_matrixes[0]).resize(size_image),image_logo,image_color_bar,pollutant,height,scale,minimal_height).save(path_map_out+"."+format_image)
